...
let option = {
  ...
  */
  ...
  process(ast) {
    ...
    _this.newLinesString = _this.buildSpacing(ast.syntax, value);
    _this.newLinesNode = gonzales.createNode({
      type: 'space',
      content: _this.newLinesString
    });
    _this.processBlock(ast);
  },
  ...
  processBlock(x) {
    ...
    if (x.is('stylesheet')) {
      // Check all @rules
      _this.processAtRules(x);

      // Check all rulesets
      _this.processRuleSets(x);
    }
  ...
  processAtRules(node) {
    node.forEach('atrule', (atRuleNode, index) => {
      option.insertNewlines(node, index);
    });
  },
  ...
  prevLineIsComment(parent, index) {
    ...
    if (parentSyntax === 'sass') {
      ...
      return (
        ...
        _this.isNewline(prevMinusOneChild) &&
        prevChild.is('space')
      );
    }

    return (_this.isComment(prevMinusOneChild) && prevChild.is('space'));
  },
  ...
  */
  ...
  insertNewlines(node, index) {
    ...
    if (prevChild && shouldInsert) {
      var _this = option;
      if (_this.prevLineIsComment(node, index) || _this.isComment(prevChild)) {
        let lastNonCommentIndex = _this.findLatestNonCommentNode(node, index);
        prevChild = node.get(lastNonCommentIndex);
      }

      if (prevChild.is('space')) {
        _this.insertNewlinesAsString(prevChild);
      } else {
        _this.insertNewlinesAsNode(prevChild);
      }
    }
  }
};
